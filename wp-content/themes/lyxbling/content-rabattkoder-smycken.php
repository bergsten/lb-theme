...
if ( 'content' == $settings['post_content'] || is_single() ) { 
    ...
    if(1 == $is_percentage) {
        ...
        $discount_image = '<a href="' . '/till/' . $post->ID . ($show_rabattkod?'?iframe=true':'') . '" rel="external" target="_blank"><img class="alignright size-thumbnail" alt="' . get_the_title() . '" src="' . get_stylesheet_directory_uri() . '/images/rabattkoder/rabatt-' . $discount_amount . '.png" width="300" height="230" /></a>';
        echo($discount_image);
    } else {
